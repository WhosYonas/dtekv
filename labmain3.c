...
int counter = 0;
int prime = 1234567;
  // Display hours/minutes/seconds on 7-segment
  int second_tens = 0;
  int second_ones = 0;
  int minute_tens = 0;
  int minute_ones = 0;
...
void handle_interrupt(unsigned cause) {
  volatile int* timer_flag = (int*)0x04000020;
  *timer_flag = *timer_flag & 0xFFFE;  // Clear TO flag

  //uppgoft f där vi runnar koden under om 10 interupts
  counter++;
...
void labinit(void){
  ...
  int ticks_per_100ms = 0b01011011100011011000000; 
  *periodL = ticks_per_100ms & 0xFFFF;
  *periodH = (ticks_per_100ms>>16) & 0xFFFF;

  //Enable timer to generate interupts (ITO = 1)
  volatile int* control = (int*)0x04000024;
  *control = (*control & 0xFFFE) + 1; //ITO is active so we enable interupts

  // Enable interrupts globally and from timer by calling enable_interrupt from boot.S function
  enable_interrupts();
}

void start_timer(){
  volatile int* control = (int*)0x04000024;
  *control = (*control & 0xFFF9) + 0b110; //ITO is active so we enable interupts
}

void custom_delay(){
  volatile int* timer_flag = (int*)0x04000020;
  start_timer();  
  while (((*timer_flag) & 0b1) == 0);
  *timer_flag = *timer_flag & 0xFFFE;  
}
